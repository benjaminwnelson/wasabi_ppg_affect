...
#Import Data
```{r}
data1 <- data.table::fread("~/Desktop/Manuscripts/WASABI HR and Affect/WASABI.csv")
```
...
data3 <- data3 %>% 
  dplyr::filter(ConditionLabel != "Pet")
```

##Turn Data Into Long
```{r}
data_long1 <- data3 %>%  
  pivot_longer(cols = c(-SID, -ConditionLabel, -Gender, -Age, -RaceEthnicity, -bdi, -gad), names_sep="_(?=[^_]+$)", names_to = c(".value", "wave"))
...
data_long2 <- data_long2 %>% 
  mutate(ConditionLabel = relevel(ConditionLabel, 3))
```
...
hr_stress_model1 <- lmer(vasstress ~ 1 + hr_cb + hr_cw + bdi + gad + ConditionLabel + Gender + (1 + hr_cw | SID), REML = TRUE, data = data_long2)
...
hr_stress_model2 <- lmer(vasstress ~ 1 + hr_cb*hr_cw + hr_cw*bdi + hr_cw*gad + hr_cw*ConditionLabel + hr_cw*Gender + (1 + hr_cw | SID), REML = TRUE, data = data_long2)

hr_stress_model2 <- lmer(vasstress ~ (1 + hr_cw)*(hr_cb + bdi + gad + ConditionLabel + Gender) + (1 + hr_cw | SID), REML = TRUE, data = data_long2)

summary(hr_stress_model2)
confint(hr_stress_model2)
performance::check_convergence(hr_stress_model2) #converged okay
performance::check_singularity(hr_stress_model2) #is not singular
performance::r2(hr_stress_model2)
performance::icc(hr_stress_model2)
ggstatsplot::ggcoefstats(hr_stress_model2)
performance::check_model(hr_stress_model2)
performance::check_collinearity(hr_stress_model2)
performance::model_performance(hr_stress_model2)
FElmer(hr_stress_model2)
FEplot(hr_stress_model2)
ICClmer(hr_stress_model2)
REplot(hr_stress_model2)
```

###Compare Models
```{r}
data_long2_compare <- na.omit(data_long2)

hr_stress_model_unconditional_compare <- lmer(vasstress ~ 1 + (1| SID), REML = TRUE, data = data_long2_compare)

hr_stress_model1_compare <- lmer(vasstress ~ 1 + hr_cb + hr_cw + bdi + gad + ConditionLabel + Gender + (1 + hr_cw | SID), REML = TRUE, data = data_long2_compare)

#These two models are equivalent
hr_stress_model2_compare <- lmer(vasstress ~ 1 + hr_cb*hr_cw + hr_cw*bdi + hr_cw*gad + hr_cw*ConditionLabel + hr_cw*Gender + (1 + hr_cw | SID), REML = TRUE, data = data_long2_compare)
...
hr_anxiety_model1 <- lmer(vasanxiety ~ 1 + hr_cb + hr_cw + bdi + gad + ConditionLabel + Gender + (1 + hr_cw | SID), REML = TRUE, data = data_long2)
...
#These two models are equivalent
hr_anxiety_model2 <- lmer(vasanxiety ~ 1 + hr_cw*hr_cb + hr_cw*bdi + hr_cw*gad +  hr_cw*ConditionLabel + hr_cw*Gender + (1 + hr_cw | SID), REML = TRUE, data = data_long2, control = lmerControl(optimizer ="Nelder_Mead"))

hr_anxiety_model2 <- lmer(vasanxiety ~ (1 + hr_cw)*(hr_cb + bdi + gad +  ConditionLabel + Gender) + (1 + hr_cw | SID), REML = TRUE, data = data_long2, control = lmerControl(optimizer ="Nelder_Mead"))

summary(hr_anxiety_model2)
confint(hr_anxiety_model2)
performance::check_convergence(hr_anxiety_model2) #converged okay
performance::check_singularity(hr_anxiety_model2) #is not singular
performance::r2(hr_anxiety_model2)
performance::icc(hr_anxiety_model2)
ggstatsplot::ggcoefstats(hr_anxiety_model2)
performance::check_model(hr_anxiety_model2)
performance::model_performance(hr_anxiety_model2)
```

###Compare Models
```{r}
hr_anxiety_model_unconditional_compare <- lmer(vasanxiety ~ 1 + (1| SID), REML = TRUE, data = data_long2_compare)

hr_anxiety_model1_compare <- lmer(vasanxiety ~ 1 + hr_cb + hr_cw + bdi + gad + ConditionLabel + Gender + (1 + hr_cw | SID), REML = TRUE, data = data_long2_compare)

#These two models are equivalent
hr_anxiety_model2_compare <- lmer(vasanxiety ~ 1 + hr_cb*hr_cw + hr_cw*bdi + hr_cw*gad + hr_cw*ConditionLabel + hr_cw*Gender + (1 + hr_cw | SID), REML = TRUE, data = data_long2_compare, control = lmerControl(optimizer ="Nelder_Mead"))
...
data_long2$gad_win <- DescTools::Winsorize(data_long2$gad, na.rm = TRUE)
data_long2$bdi_win <- DescTools::Winsorize(data_long2$bdi, na.rm = TRUE)
```
...
##HR Predicting Stress
```{r}
hr_stress_model_win <- lmer(vasstress ~ 1 + hr_win_cb + hr_win_cw + bdi_win + gad_win + ConditionLabel + Gender + (1 + hr_win_cw | SID), REML = TRUE, data = data_long3, control = lmerControl(optimizer ="Nelder_Mead"))
...
##HR Predicting Anxiety
```{r}
hr_anxiety_model_win <- lmer(vasanxiety ~ 1 + hr_win_cb + hr_win_cw + bdi_win + gad_win + ConditionLabel + Gender + (1 + hr_win_cw | SID), REML = TRUE, data = data_long3, control = lmerControl(optimizer ="Nelder_Mead"))
...
#Association Between BDI and GAD with HR
```{r}
