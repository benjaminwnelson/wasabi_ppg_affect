...
#Import Data
```{r}
data1 <- data.table::fread("~/Desktop/Manuscripts/WASABI/WASABI Data.csv")
```
...
```{r}
data_long1 <- data3 %>%  
  pivot_longer(cols = c(-SID, -ConditionLabel, -Gender, -Age, -RaceEthnicity, -gadtotal, -bditotal), names_sep="_(?=[^_]+$)", names_to = c(".value", "wave"))
```
...
data_long1[,1:5] <- lapply(data_long1[,1:5], as.factor)
data_long1[,8] <- lapply(data_long1[,8], as.factor)

#Make Variables Numeric
data_long1[,6:7] <- lapply(data_long1[,6:7], as.numeric)
data_long1[,9:11] <- lapply(data_long1[,9:11], as.numeric)
```
...
hr_trajectory <- ggplot(data=subset(data_long2), aes(x = wave, y = hr, color = ConditionLabel)) +
  ...
  stat_summary(data=subset(data_long2), aes(group=ConditionLabel), fun=mean, geom="smooth") + 
...
stress_trajectory <- ggplot(data=subset(data_long2), aes(x = wave, y = vasstress, color = ConditionLabel)) +
  geom_line(aes(group = SID), alpha = .1) + #This fades the individual lines
...
anxiety_trajectory <- ggplot(data=subset(data_long2), aes(x = wave, y = vasanxiety, color = ConditionLabel)) +
  geom_line(aes(group = SID), alpha = .1) + #This fades the individual lines
  geom_point(position=position_jitter(h=0.05, w=0.05),
             shape = 21, alpha = 0.5, size = 1) +
  stat_summary(data=subset(data_long2), aes(group=ConditionLabel), fun=mean, geom="smooth") + 
...
##Heart Rate
```{r}
sumrepdat_hr <- Rmisc::summarySE(data_long2, measurevar = "hr",
                              groupvars=c("ConditionLabel", "wave"))

hr_plot2 <- ggplot(data_long2, aes(x = wave, y = hr, fill = ConditionLabel)) +
...
###Stress
```{r}
sumrepdat_stress <- Rmisc::summarySE(data_long2, measurevar = "vasstress",
                              groupvars=c("ConditionLabel", "wave"))

stress_plot2 <- ggplot(data_long2, aes(x = wave, y = vasstress, fill = ConditionLabel)) +
  PupillometryR::geom_flat_violin(aes(fill = ConditionLabel), position = position_nudge(x = .2, y = 0), adjust = 1.5, trim = FALSE, alpha = .5, colour = NA) +
...
##Anxiety
```{r}
sumrepdat_anxiety <- Rmisc::summarySE(data_long2, measurevar = "vasanxiety",
                              groupvars=c("ConditionLabel", "wave"))

anxiety_plot2 <- ggplot(data_long2, aes(x = wave, y = vasanxiety, fill = ConditionLabel)) +
  PupillometryR::geom_flat_violin(aes(fill = ConditionLabel), position = position_nudge(x = .2, y = 0), adjust = 1.5, trim = FALSE, alpha = .5, colour = NA) +
...
data_long3$hr_zscore <- scale(data_long3$hr)
data_long3$stress_zscore <- scale(data_long3$vasstress)
data_long3$anxiety_zscore <- scale(data_long3$vasanxiety)
str(data_long3)
```

##Turn Data Into Long
```{r}
hr_stress_data <- data_long3 %>%  
...
##Turn Data Into Long
```{r}
hr_stressanxiety_data <- data_long3 %>%  
...
```{r}
hr_stress_model_unconditional <- lmer(vasstress ~ 1 + hr_cb + hr_cw + (1| SID), REML = TRUE, data = data_long3)

hr_stress_model1 <- lmer(vasstress ~ 1 + hr_cb + hr_cw*bditotal + hr_cw*gadtotal + ConditionLabel + Gender + (1 + hr_cw | SID), REML = TRUE, data = data_long3)

#These two models are equivalent
hr_stress_model2 <- lmer(vasstress ~ 1 + hr_cb + hr_cw*bditotal + hr_cw*gadtotal + hr_cw*ConditionLabel + hr_cw*Gender + (1 + hr_cw | SID), REML = TRUE, data = data_long3, control = lmerControl(optimizer ="Nelder_Mead"))

hr_stress_model2 <- lmer(vasstress ~ (1 + hr_cw)*(hr_cb + bditotal + gadtotal + ConditionLabel + Gender) + (1 + hr_cw | SID), REML = TRUE, data = data_long3)
...
hr_stress_model_unconditional_compare <- lmer(vasstress ~ 1 + (1| SID), REML = TRUE, data = data_long3_compare)

hr_stress_model1_compare <- lmer(vasstress ~ 1 + hr_cb + hr_cw*bditotal + hr_cw*gadtotal + ConditionLabel + Gender + (1 + hr_cw | SID), REML = TRUE, data = data_long3_compare)

#These two models are equivalent
hr_stress_model2_compare <- lmer(vasstress ~ 1 + hr_cb + hr_cw*bditotal + hr_cw*gadtotal + hr_cw*ConditionLabel + hr_cw*Gender + (1 + hr_cw | SID), REML = TRUE, data = data_long3_compare, control = lmerControl(optimizer ="Nelder_Mead"))
...
```{r}
hr_anxiety_model_unconditional <- lmer(vasanxiety ~ 1 +  (1 | SID), REML = TRUE, data = data_long3)

hr_anxiety_model1 <- lmer(vasanxiety ~ 1 + hr_cb + hr_cw*bditotal + hr_cw*gadtotal + ConditionLabel + Gender + (1 + hr_cw | SID), REML = TRUE, data = data_long3)

#These two models are equivalent
hr_anxiety_model2 <- lmer(vasanxiety ~ 1 + hr_cb + hr_cw*hr_cb + hr_cw*bditotal + hr_cw*gadtotal + hr_cw*ConditionLabel + hr_cw*Gender + (1 + hr_cw | SID), REML = TRUE, data = data_long3)

hr_anxiety_model2 <- lmer(vasanxiety ~ (1 + hr_cw)*(hr_cb + bditotal + gadtotal + ConditionLabel + Gender) + (1 + hr_cw | SID), REML = TRUE, data = data_long3)
...
###Compare Models
```{r}
hr_anxiety_model_unconditional_compare <- lmer(vasanxiety ~ 1 + (1| SID), REML = TRUE, data = data_long3_compare)

hr_anxiety_model1_compare <- lmer(vasanxiety ~ 1 + hr_cb + hr_cw*bditotal + hr_cw*gadtotal + ConditionLabel + Gender + (1 + hr_cw | SID), REML = TRUE, data = data_long3_compare)

#These two models are equivalent
hr_anxiety_model2_compare <- lmer(vasanxiety ~ 1 + hr_cb + hr_cw*bditotal + hr_cw*gadtotal + hr_cw*ConditionLabel + hr_cw*Gender + (1 + hr_cw | SID), REML = TRUE, data = data_long3_compare, control = lmerControl(optimizer ="Nelder_Mead"))
...
tab_model(hr_stress_model1, hr_anxiety_model1,
          ...
          digits = 3,
          show.aic = TRUE,
          file = "~/Desktop/Manuscripts/WASABI HR and Affect/Output.doc")
```

#Sensitivity Analyses
##Winsorize
```{r}
data_long3$hr_win <- DescTools::Winsorize(data_long3$hr, na.rm = TRUE)
data_long3$vasstress_win <- DescTools::Winsorize(data_long3$vasstress, na.rm = TRUE)
data_long3$vasanxiety_win <- DescTools::Winsorize(data_long3$vasanxiety, na.rm = TRUE)
data_long3$gadtotal_win <- DescTools::Winsorize(data_long3$gadtotal, na.rm = TRUE)
data_long3$bditotal_win <- DescTools::Winsorize(data_long3$bditotal, na.rm = TRUE)
```

##Analysis
###Create Between and Within Person Terms
```{r}
data_long4 <- bmlm::isolate(data_long3, by = "SID",
...
##HR Predicting Stress
```{r}
hr_stress_model_win <- lmer(vasstress ~ 1 + hr_win_cb + hr_win_cw*bditotal_win + hr_win_cw*gadtotal_win + ConditionLabel + Gender + (1 + hr_win_cw | SID), REML = TRUE, data = data_long4, control = lmerControl(optimizer ="Nelder_Mead"))
summary(hr_stress_model_win)
confint(hr_stress_model_win)
performance::check_convergence(hr_stress_model_win) #converged okay
performance::check_singularity(hr_stress_model_win) #is not singular
performance::r2(hr_stress_model_win)
performance::icc(hr_stress_model_win)
ggstatsplot::ggcoefstats(hr_stress_model_win)
performance::check_model(hr_stress_model_win)
performance::model_performance(hr_stress_model_win)
FElmer(hr_stress_model_win)
FEplot(hr_stress_model_win)
ICClmer(hr_stress_model_win)
REplot(hr_stress_model_win)
```

##HR Predicting Anxiety
```{r}
hr_anxiety_model_win <- lmer(vasanxiety ~ 1 + hr_win_cb + hr_win_cw*bditotal_win + hr_win_cw*gadtotal_win +  ConditionLabel + Gender + (1 + hr_win_cw | SID), REML = TRUE, data = data_long4, control = lmerControl(optimizer ="Nelder_Mead"))
summary(hr_anxiety_model_win)
confint(hr_anxiety_model_win)
performance::check_convergence(hr_anxiety_model_win) #converged okay
performance::check_singularity(hr_anxiety_model_win) #is not singular
performance::r2(hr_anxiety_model_win)
performance::icc(hr_anxiety_model_win)
ggstatsplot::ggcoefstats(hr_anxiety_model_win)
performance::check_model(hr_anxiety_model_win)
performance::model_performance(hr_anxiety_model_win)
```

##Create Table
```{r}
tab_model(hr_stress_model_win, hr_anxiety_model_win,
          ...
          digits = 3,
          show.aic = TRUE,
          file = "~/Desktop/Manuscripts/WASABI HR and Affect/Output_Winsorized.doc")
```
